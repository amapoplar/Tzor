InstallTzor[] := 
 Module[{$downLoadURL = 
  ...
  PrintTemporary[
   "Downloading tzor from github to " <> $UserDocumentsDirectory <> 
    "..."];
  zipFile = 
   URLDownload[$downLoadURL , 
    FileNameJoin[{$UserDocumentsDirectory, "tzor.zip"}]];
  PrintTemporary["Unzipping tzor.zip..."];
  unzipFile = 
   If[Or @@ (StringContainsQ[#, "Tzor-main"] & /@ 
       FileNames[All, $UserDocumentsDirectory]),
    				If[               
     ChoiceDialog[
      "There has been a FileDirectory named Tzor-main in " <> \
$UserDocumentsDirectory <> 
       ", do you want overwrite it or qiut installation?", \
{"OverWrite" -> True, "Quit" -> False}],
...
{"OverWrite" -> True, "Quit" -> False}],
     ...
     FileNameJoin[{$UserBaseDirectory, "Applications/tzor"}]],
    			DeleteDirectory[
     FileNameJoin[{$UserDocumentsDirectory, "Tzor-main"}], 
     DeleteContents -> True]; Return[]
    		];
   CopyDirectory[
    FileNameJoin[{$UserDocumentsDirectory, "Tzor-main/tzor"}], 
    FileNameJoin[{$UserBaseDirectory, "Applications/tzor"}]]];
