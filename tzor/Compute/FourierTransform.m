...
Format[partial[p_,index_],TraditionalForm]:= DisplayForm[FractionBox["\[PartialD]",RowBox[{"\[PartialD]",SubscriptBox[p,index]}]]]
...
fourierTransform[func0_,x0_,p0_]:=Module[{fun = func0,s= -Exponent[func0,scalarP[x0,x0]],x=x0,p=p0,k,slashindex = Nothing,extrLorenz = 1,nLorenz ={},tempresult=0},
If[(s/.dim->4)<=0,k = 1-s/.dim->4;s= 1;nLorenz = {ToExpression["\[Mu]Ext"<>ToString[#]],ToExpression["\[Nu]Ext"<>ToString[#]]}&/@Range[k];extrLorenz = Times@@(metric[#[[1]],#[[2]]]&/@nLorenz);
extrLorenz =extrLorenz Times@@(partial[p,#]&/@Flatten[nLorenz])];
tempresult= fourierMasterFunction[s,p]//Expand;
fun =fun/.{scalarP[x,x]->1,scalarP[x,k_?(#=!=x&)]:>fourVector[x,indexNew["\[Mu]"]]fourVector[k,indexNew["\[Mu]","EndQ"->True]],slash[x]:> fourVector[x,indexNew["\[Mu]"]]gamma[indexNew["\[Mu]","EndQ"->True]]};
nLorenz = giveIndice[fun,x];
fun = fun/.{fourVector[x,_]:>1};
tempresult =(extrLorenz If[Length[nLorenz]>0,Fold[ Expand[-I DLorenz[#1,p,#2]]&,tempresult*fun,nLorenz ],tempresult*fun]//Expand)//.{metric[\[Mu]_,\[Nu]_]fourVector[p1_,\[Mu]_]:>fourVector[p1,\[Nu]],fourVector[k1_,\[Mu]_]fourVector[k2_,\[Mu]_]:>scalarP[k1,k2],gamma[\[Mu]_]fourVector[p1_,\[Mu]_]:>slash[p1],metric[\[Mu]_,\[Nu]_]^2:>dim,fourVector[p1_,\[Mu]_]^2:> scalarP[p1,p1],metric[\[Mu]1_,\[Mu]2_]metric[\[Mu]2_,\[Mu]3_]:>metric[\[Mu]1,\[Mu]3]}
]
